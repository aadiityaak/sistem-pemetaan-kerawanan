                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                                {{ partaiPolitik.data.filter((p) => p.status_aktif).length }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase dark:text-gray-300">Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase dark:text-gray-300">
                                Data Suara
                            </th>
                            <th class="px-6 py-3 text-right text-xs font-medium tracking-wider text-gray-500 uppercase dark:text-gray-300">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                        <tr v-for="partai in partaiPolitik.data" :key="partai.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4 text-sm font-medium whitespace-nowrap text-gray-900 dark:text-gray-100">
                                <span
                                    class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300"
                                >
                                    {{ partai.nomor_urut }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                <div class="flex items-center">
                                    <img
                                        v-if="partai.logo_url"
                                        :src="partai.logo_url"
                                        :alt="partai.nama_partai"
                            ...
                            <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-900 dark:text-gray-100">
                                <span
                                    class="inline-flex rounded-full bg-gray-100 px-2 text-xs leading-5 font-semibold text-gray-800 dark:bg-gray-700 dark:text-gray-300"
                                >
                                    {{ partai.singkatan }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                <div v-if="partai.nama_ketua || partai.foto_ketua_url" class="flex items-center">
                                    <img
                                        v-if="partai.foto_ketua_url"
                                        :src="partai.foto_ketua_url"
                                        :alt="partai.nama_ketua || 'Ketua Umum'"
                                        class="mr-3 h-8 w-8 rounded-full object-cover"
                                    />
                                    <div class="mr-3 h-8 w-8 rounded-full bg-gray-300 dark:bg-gray-600" v-else></div>
                                    <div>
                                ...
                                <div v-else class="text-xs text-gray-500 italic dark:text-gray-400">Belum diisi</div>
                            </td>
                            <td class="px-6 py-4 text-sm whitespace-nowrap">
                                <span
                                    v-if="partai.status_aktif"
                                    class="inline-flex rounded-full bg-green-100 px-2 text-xs leading-5 font-semibold text-green-800 dark:bg-green-900 dark:text-green-300"
                                >
                                    Aktif
                                </span>
                                <span
                                    v-else
                                    class="inline-flex rounded-full bg-red-100 px-2 text-xs leading-5 font-semibold text-red-800 dark:bg-red-900 dark:text-red-300"
                                >
                                    Tidak Aktif
                                </span>
                            </td>
                        ...
                        <div>
                            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                <template v-for="(link, index) in partaiPolitik.links" :key="index">
                                    ...
                                    <span
                                        v-else
                                        class="relative inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400"
                                        :class="[index === 0 ? 'rounded-l-md' : '', index === partaiPolitik.links.length - 1 ? 'rounded-r-md' : '']"
                                        v-html="link.label"
                                    ></span>
                                </template>
                            </nav>
