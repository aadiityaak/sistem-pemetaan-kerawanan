...
const saveEvent = async () => {
    ...
    try {
        ...
        if (eventForm.value.start_date && eventForm.value.end_date && new Date(eventForm.value.end_date) < new Date(eventForm.value.start_date)) {
...
const hasUnsavedChanges = computed(() => {
    ...
    return form.title.trim() !== '' || form.description.trim() !== '' || form.start_date !== '' || form.end_date !== '' || form.color !== '#3B82F6';
});
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
                ...
                <div class="border-b border-gray-200 p-6 dark:border-gray-700">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-4">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ currentMonth }} {{ currentYear }}</h2>
                        </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                            ...
                            <div v-if="props.eventFilter === 'agenda'" class="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    class="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                >
                    ...
                    <div class="grid grid-cols-7 gap-1">
                        <div
                            ...
                            :class="{
                                ...
                                'border-red-200 bg-red-50 dark:border-red-700 dark:bg-red-900': day.holidays.some((h) => h.isNationalHoliday),
                                'border-orange-200 bg-orange-50 dark:border-orange-700 dark:bg-orange-900':
                                    day.holidays.some((h) => !h.isNationalHoliday) && !day.holidays.some((h) => h.isNationalHoliday),
                            }"
                        ...
                        >
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div v-if="day.allItems.length > 0" class="flex items-center gap-1">
                                    <div
                                        ...
                                        class="rounded-full bg-blue-100 px-1.5 py-0.5 text-xs font-medium text-blue-600 dark:bg-blue-900 dark:text-blue-400"
                                    >
                                        {{ day.events.length }}
                                    </div>
                                    <div
                                        ...
                                        :class="
                                            day.holidays.some((h) => h.isNationalHoliday)
                            ...
                            <div class="mt-1 space-y-1">
                                ...
                                <div
                                    ...
                                    :key="holiday.id"
                                    class="cursor-default truncate rounded px-2 py-1 text-xs font-medium text-white"
                                    :style="{ backgroundColor: holiday.color }"
                                    :title="`${holiday.title} - ${holiday.description}`"
                                >
                                    <span class="block truncate">🏛️ {{ holiday.title }}</span>
                                </div>
                                ...
                                <div
                                    ...
                                    :key="event.id"
                                    class="event-item group relative cursor-pointer truncate rounded px-2 py-1 text-xs font-medium text-white transition-all hover:scale-105 hover:shadow-md"
                                    :style="{ backgroundColor: event.color }"
                                    :title="`${event.title}${event.description ? ' - ' + event.description : ''}`"
                                    @click.stop="editEvent(event)"
                                >
                                    <span class="block truncate pr-4">{{ event.title }}</span>
                                    <!-- Event actions on hover -->
                                    <div class="absolute top-0 right-0 hidden h-full items-center group-hover:flex">
                                        <button
                                            @click.stop="deleteEvent(event)"
                                            class="ml-1 rounded-full bg-red-500 p-1 text-white shadow-sm transition-colors hover:bg-red-600"
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    >
                            ...
                            <div class="mt-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        :disabled="isLoading"
                                        class="mt-1 block w-full rounded-md border px-3 py-2 shadow-sm transition-colors focus:ring-2 focus:outline-none"
                                        :class="{
                                            ...
                                            'cursor-not-allowed bg-gray-100 dark:bg-gray-600': isLoading,
                                ...
                                <!-- Description -->
                                <div>
                                    <label for="description-input" class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                                        >Deskripsi</label
                                    >
                                    <textarea
                                        ...
                                        rows="3"
                                        :disabled="isLoading"
                                        class="mt-1 block w-full resize-none rounded-md border px-3 py-2 shadow-sm transition-colors focus:ring-2 focus:outline-none"
                                        :class="{
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 dark:bg-gray-700">
                            <Button @click="saveEvent" :disabled="isLoading" class="w-full sm:ml-3 sm:w-auto">
                                <svg
                                    v-if="isLoading"
                            ...
                            <Button @click="closeModal(false)" variant="outline" :disabled="isLoading" class="mt-3 w-full sm:mt-0 sm:w-auto">
                                Batal
                            </Button>
                        </div>
                    </div>
        ...
        >
            <!-- Modal backdrop -->
            <div class="fixed inset-0 bg-gray-500/75 backdrop-blur-sm transition-opacity" aria-hidden="true" @click="closeDayModal()"></div>
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    >
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 dark:bg-gray-800">
                            <div class="flex items-center justify-between">
                                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="day-modal-title">
                            ...
                            <div class="mt-6 space-y-4">
                                ...
                                <div v-if="selectedDayHolidays.length > 0">
                                    <h4 class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">🏛️ Hari Libur</h4>
                                    <div class="space-y-2">
                                        <div
                                        ...
                                        >
                                            ...
                                            <div class="h-4 w-4 flex-shrink-0 rounded-full" :style="{ backgroundColor: holiday.color }"></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Events Section -->
                                <div>
                                    <h4 class="mb-3 flex items-center justify-between text-sm font-medium text-gray-700 dark:text-gray-300">
                                        📅 Event ({{ selectedDayEvents.length }})
                                    </h4>

                                    <div v-if="selectedDayEvents.length === 0" class="py-8 text-center">
                                    ...
                                    <div v-else class="space-y-3">
                                        <div
                                            ...
                                            :key="event.id"
                                            class="group flex items-center justify-between rounded-lg border border-gray-200 bg-white p-4 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
                                        >
                                            <div class="flex flex-1 items-center space-x-3">
                                                <div class="h-4 w-4 flex-shrink-0 rounded-full" :style="{ backgroundColor: event.color }"></div>
                                                <div class="min-w-0 flex-1">
                                                    ...
                                                    <p v-if="event.description" class="mt-1 truncate text-sm text-gray-600 dark:text-gray-400">
                                                        {{ event.description }}
                                                    </p>
                                                </div>
                                            </div>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 dark:bg-gray-700">
                            <div class="flex justify-end">
                                <Button @click="closeDayModal()" variant="outline"> Tutup </Button>
                            </div>
                        </div>
                    </div>
                </div>
